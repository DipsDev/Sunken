...
export default function App() {
  ...
  async function handleTrainerFinish(
    entry: Entry,
    shareTexts: string[] | null,
    gridData: Grid | null,
    codePath: CodePath | null,
    ...
    setDoneData({ entry, shareTexts, gridData, codePath });
    setScreen("done");
  }
  ...
  return (
    <div className="wrap">
      ...
      {screen === "trainer" && draft && (
        <Trainer draft={draft} onFinish={handleTrainerFinish} />
      )}
      {screen === "done" && doneData && (
        <Done
          entry={doneData.entry}
          shareTexts={doneData.shareTexts}
          gridData={doneData.gridData}
          codePath={doneData.codePath}
          onDone={goHome}
        />
      )}
      {screen === "recover" && recoverLabel && <Recover onBack={goHome} />}
    </div>
  );
}
